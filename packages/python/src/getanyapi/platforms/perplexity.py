...
class PerplexityBrandVisibilityInput(TypedDict, total=False):
    """Input for Perplexity Brand Visibility."""
...
class PerplexityBrandVisibilityData(BaseModel):
    ...
    citation_rank: int | None = Field(
        default=None,
        alias="citationRank",
        description="One-based citation rank for the brand, or null when it was not cited.",
    )
    citations: list[PerplexityBrandVisibilityCitation] | None = Field(
        default=None,
    ...
    cited_urls: list[str] | None = Field(
        default=None,
        alias="citedUrls",
        description="URLs attributed to the brand domain.",
    )
    competitors: list[PerplexityBrandVisibilityCompetitor] | None = Field(
        default=None,
    ...
    model: str | None = Field(
        default=None,
        description="Perplexity model that generated the answer. Populated whenever the provider has data for the entity. Present whenever the upstream returns this record.",
    )
    ...
    web_search_triggered: bool | None = Field(
        ...
        description="Whether Perplexity used web search for the answer. Populated whenever the provider has data for the entity. Present whenever the upstream returns this record.",
    )


class PerplexityBrandVisibilityCitation(BaseModel):
    ...
    last_updated_utc: float | None = Field(
        default=None,
        alias="lastUpdatedUtc",
        description="Source last-updated UTC epoch timestamp in seconds (Unix time). Multiply by 1000 for a JS Date in milliseconds.",
    )
    matched_text: str | None = Field(
        default=None,
        alias="matchedText",
        description="Answer text matched to the citation evidence. Populated whenever the provider has data for the entity. Present whenever the upstream returns this record.",
    )
    ...
    published_utc: float | None = Field(
        default=None,
        alias="publishedUtc",
        description="Source published UTC epoch timestamp in seconds (Unix time). Multiply by 1000 for a JS Date in milliseconds. Populated whenever the provider has data for the entity. Present whenever the upstream returns this record.",
    )
    ref_index: int | None = Field(
        default=None,
        alias="refIndex",
        description="One-based citation reference index used in the answer. Populated whenever the provider has data for the entity. Present whenever the upstream returns this record.",
    )
    ...
    source_type: str | None = Field(
        default=None,
        alias="sourceType",
        description="Source evidence type. Populated whenever the provider has data for the entity. Present whenever the upstream returns this record.",
    )
...
class PerplexityBrandVisibilityCompetitor(BaseModel):
    model_config = ConfigDict(extra="allow", populate_by_name=True)
    ...
    first_position: int | None = Field(
        default=None,
        alias="firstPosition",
        description="Character position of the first competitor mention when present.",
    )
    mention_count: int | None = Field(
        default=None,
        alias="mentionCount",
...
class PerplexityNamespace:
    ...
    def brand_visibility(
        ...
        **input: Unpack[PerplexityBrandVisibilityInput],
    ) -> RunResult[PerplexityBrandVisibilityData]:
        """Perplexity Brand Visibility

        Analyze how Perplexity mentions and cites a brand relative to its
        competitors.

        Price: $0.0045 per request.

        Example:
            res = client.perplexity.brand_visibility(brand="OpenAI", competitors=["Anthropic", "Google DeepMind", "Meta"], country="US", domain="openai.com", prompt="What is OpenAI and who are its main competitors?")
        """
        raw = self._client._run_raw(  # pyright: ignore[reportPrivateUsage]
            "perplexity.brand_visibility", dict(input), options
        )
        return RunResult[PerplexityBrandVisibilityData].model_validate(raw)
